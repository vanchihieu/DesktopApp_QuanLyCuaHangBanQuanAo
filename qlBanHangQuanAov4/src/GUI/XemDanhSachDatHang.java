...
//		Set color scrollbar thumb
		...
		tableModel = new DefaultTableModel(new String[] { "Mã phiếu đặt hàng","Ngày lập","Người lập","Khách hàng","Số điện thoại", "Tổng tiền"}, 0);
		...
		tbl.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				for (ChiTietPhieuDatHang ctpdh : ChiTietPhieuDatHang_dao.getAllChiTietPhieuDatHang(tbl.getValueAt(row, 0).toString())) {
					...
					Object[] data = {ctpdh.getSanPham().getMaSanPham(),getTenSP(ctpdh.getSanPham().getMaSanPham()), ctpdh.getSoLuong(),
							format.format(ctpdh.getDonGia()),Math.round(phanTram),
							format.format(Integer.valueOf(ctpdh.getSoLuong())*Double.valueOf(ctpdh.getDonGia()))};
...
//		Set color scrollbar thumb
		...
		tableModelCTPDH = new DefaultTableModel(new String[] { "Mã sản phẩm", "Tên sản phẩm","Số lượng","Đơn giá","Giảm giá","Thành tiền" }, 0);
...
//			JOptionPane.showMessageDialog(this, ChecktaoSQL());
			...
			LapHoaDon lhd = new LapHoaDon(main, maTK);
			for (KhachHang kh : KhachHang_dao.getAllKhachHang()) {
			...
			for(int i = 0;i < toTalRowCHPDH;i++) {
				Object[] data = {tblCTPDH.getValueAt(i, 0), tblCTPDH.getValueAt(i, 1), tblCTPDH.getValueAt(i, 2),
						tblCTPDH.getValueAt(i, 3), tblCTPDH.getValueAt(i, 4), tblCTPDH.getValueAt(i, 5)};
	...
	public Double getDonGia(String ma) {
		for (SanPham sanPham : SanPham_dao.getAllSanPham()) {
		...
		try {
			...
			while (rs.next()) {
				Object[] data = { rs.getString(1), rs.getString(2), rs.getString(3), 
						rs.getString(4), rs.getString(5), format.format(Double.valueOf(rs.getString(6))) };
				tableModel.addRow(data);
			}
	...
	public void findDuLieu(String sql) {
		ConnectDB.getInstance();
		Connection con = ConnectDB.getConnection();
		try {
			Statement statement = con.createStatement();
			...
			while (rs.next()) {
				Object[] data = { rs.getString(1), rs.getString(2), rs.getString(3), 
						rs.getString(4), rs.getString(5), Math.round(Double.valueOf(rs.getString(6))) };
				tableModel.addRow(data);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public String ChecktaoSQL() {
		...
		if(!txtSDT.getText().isEmpty()) {
			having +=" kh.soDienThoai = '" + txtSDT.getText()+"'";
		}
		if(!txtNgayLap.getText().isEmpty()) {
			if(having.contains("kh.soDienThoai"))
				having += " and pdh.ngayLap = '" + txtNgayLap.getText()+"'";
			else
				having +=" pdh.ngayLap = '" + txtNgayLap.getText()+"'";
		}
		if(!txtNguoiLap.getText().isEmpty()) {
			if(having.contains("and") || sql.contains("kh.soDienThoai") || sql.contains("pdh.ngayLap"))
				having += " and nv.tenNhanVien = N'" + txtNguoiLap.getText()+"'";
			else
				having += " nv.tenNhanVien = N'" + txtNguoiLap.getText()+"'";
		}
		if(!txtTenKH.getText().isEmpty()) {
			if(sql.contains("and") || sql.contains("kh.soDienThoai") || sql.contains("pdh.ngayLap")
					|| having.contains("nv.tenNhanVien"))
				having += " and kh.tenKhachHang = N'" + txtTenKH.getText()+"'";
			else
				having += " kh.tenKhachHang = N'" + txtTenKH.getText()+"'";
		}
