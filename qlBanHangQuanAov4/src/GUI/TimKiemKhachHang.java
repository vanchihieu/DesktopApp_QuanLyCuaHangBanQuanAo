...
public class TimKiemKhachHang extends JPanel implements ActionListener{
	 ...
	 * Create the panel.
	 */
	public TimKiemKhachHang(ManHinhChinh main) {
		this.main = main;
...
//		top += padding + 55;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(src.equals(btnSearch)) {
				...
				for (KhachHang kh : KhachHang_dao.Find(sql)) {
					Object[] data = {kh.getMaKhachHang(), kh.getTenKhachHang(), kh.isGioiTinh()==true?"nam":"nữ",
							kh.getDiaChi(), kh.getSoDienThoai(), kh.getEmail()};
					tableModel.addRow(data);
				}
//			JOptionPane.showMessageDialog(this, sql);
	...
	public String ChecktaoSQL() {
		...
		if(!txtMa.getText().isEmpty()) {
			sql +=" maKhachHang = '" + txtMa.getText()+"'";
		}
		if(!txtTen.getText().isEmpty()) {
			if(sql.contains("maKhachHang"))
				sql += " and tenKhachHang = N'" + txtTen.getText()+"'";
			else
				sql +=" tenKhachHang = N'" + txtTen.getText()+"'";
		}
		if(!txtPhone.getText().isEmpty()) {
			if(sql.contains("and") || sql.contains("maKhachHang") || sql.contains("tenKhachHang"))
				sql += " and soDienThoai = '" + txtPhone.getText()+"'";
			else
				sql += " soDienThoai = '" + txtPhone.getText()+"'";
		}
		if(!txtAddress.getText().isEmpty()) {
			if(sql.contains("and") || sql.contains("maKhachHang") || sql.contains("tenKhachHang")
					|| sql.contains("soDienThoai"))
				sql += " and diaChi = N'" + txtAddress.getText()+"'";
			else
				sql += " diaChi = N'" + txtAddress.getText()+"'";
		}
