...
public class XemDanhSachPhieuNhap extends JPanel implements ActionListener{
	...
	public XemDanhSachPhieuNhap(ManHinhChinh main) {
		this.main= main;
...
//		Set color scrollbar thumb
		...
		for (PhieuNhap pn : PhieuNhap_dao.getAllPhieuNhap()) {
			Object[] data = {pn.getMaPhieuNhap(),getTenNV(pn.getNguoiLap().getMaNhanVien()),pn.getNgayLap(), pn.getNhaCungCap()};
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(src.equals(btnTimKiem)) {
			...
			if(rdoNguoiLap.isSelected()) {
				...
				for (PhieuNhap pn : PhieuNhap_dao.getAllPhieuNhap()) {
					if(pn.getNguoiLap().getMaNhanVien().equals(nguoilap)) {
						Object[] data = {pn.getMaPhieuNhap(),getTenNV(pn.getNguoiLap().getMaNhanVien()),pn.getNgayLap(), pn.getNhaCungCap()};
						tableModel.addRow(data);
					}
				}
