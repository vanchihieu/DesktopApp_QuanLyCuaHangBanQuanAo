...
public class LapHoaDon extends JPanel implements ActionListener{
	...
	public LapHoaDon(ManHinhChinh main, String maTK) {
		this.maTK = maTK;
		this.main = main;
		tongTien = 0.0;
...
//		txtSoDienThoai.setEditable(false);
		...
		scrSP.setBounds(left, 500, 1400, 230);
...
//		this.add(pnlControlSP);
		...
		scrCTHD.setBounds(left+800, 80, 650, 200);
...
//		this.add(pnlControlCTHD);
		...
		pnlRow5.setBounds(left+850, 315, 792, 120);
		top += padding+50;
		add(pnlRow5);
		...
		pnlActions.setBounds(left, top, 1450, 80);
		top += padding;
		add(pnlActions);
...
//		pnlActions.add(cboDonDatHang);
		...
		btnThem.setBounds(550, 0, 180, 40);
		...
		btnRefresh.setBounds(850, 0, 150, 40);
		...
		btnScan.setBounds(1050, 0, 280, 40);
			...
			if(tableModelCTHD.getRowCount() == 0) {
				...
				if(validator()) {
					...
					HoaDon hd = new HoaDon(mahd, LocalDate.of(Integer.valueOf(date[0]), Integer.valueOf(date[1]), Integer.valueOf(date[2])),
							new NhanVien(nguoilap), new KhachHang(khachhang));
					...
					if(HoaDon_dao.create(hd)) {
						for(int i = 0; i < row;i++) {
							...
							ChiTietHoaDon cthd = new ChiTietHoaDon(new HoaDon(mahd),
									new SanPham(masp), soluong, dongia);
...
//				JOptionPane.showMessageDialog(this, "Bạn chưa chọn đơn đặt hàng để xem"); 
			...
			XemDanhSachDatHang dsDH = new XemDanhSachDatHang(main, maTK); 
//			JPanel pnl = new QuanLyKhachHang(main);
			main.addPnlBody(dsDH, title, 2, 1);
		}
		else if(src.equals(btnScan)) {
			Form_Scan FScan = new Form_Scan(this);
