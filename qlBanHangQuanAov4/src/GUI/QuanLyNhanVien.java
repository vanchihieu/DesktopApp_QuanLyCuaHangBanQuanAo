...
public class QuanLyNhanVien extends JPanel implements ActionListener{
	...
	public QuanLyNhanVien(ManHinhChinh main) {
		try {
...
//			JOptionPane.showMessageDialog(this, taoMaNhanVien());
			if(validator()) {
				...
				NhanVien nv = new NhanVien(maNhanVien, tenNhanVien, LocalDate.of(Integer.valueOf(date[0]), Integer.valueOf(date[1]), Integer.valueOf(date[2])),
						gioiTinh, diaChi, soDienThoai, quanLy==null?new NhanVien():new NhanVien(getMaQL(quanLy)), getMaLNV(loaiNhanVien), luong, getMaCL(caLam));
			...
			if(row != -1) {
				if(validator()) {
					...
					String loaiNhanVien = cboLoaiNhanVien.getSelectedItem().toString();
					String quanLy = "";
					if(cboQuanLy.getSelectedItem() == null)
						quanLy = null;
					else
						quanLy = cboQuanLy.getSelectedItem().toString(); 
					Double luong = chuyenDoiFormat(txtLuong.getText());
					String caLam = cboCaLam.getSelectedItem().toString();
					NhanVien nv = new NhanVien(maNhanVien, tenNhanVien, LocalDate.of(Integer.valueOf(date[0]), Integer.valueOf(date[1]), Integer.valueOf(date[2])),
							gioiTinh, diaChi, soDienThoai, quanLy==null?new NhanVien():new NhanVien(getMaQL(quanLy)), getMaLNV(loaiNhanVien), luong, getMaCL(caLam));
