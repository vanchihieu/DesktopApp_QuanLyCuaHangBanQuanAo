...
public class LapPhieuNhap extends JPanel implements ActionListener{
	...
	public LapPhieuNhap(ManHinhChinh main, String maTK) {
		this.main = main;
		...
		scrCTKM.setBounds(left, 300, 450, 150);
		...
		if(src.equals(btnAddSP)) {
			...
			if(tableModelCTPN.getRowCount() != 0) {
				if(validator()) {
					...
					PhieuNhap pn = new PhieuNhap(mapn, LocalDate.of(Integer.valueOf(date[0]), Integer.valueOf(date[1]), Integer.valueOf(date[2])), new NhanVien(maNV), nhaCungCap);
					if(PhieuNhap_dao.create(pn)) {
						for(int i = 0;i < row;i++) {
							...
							ChiTietPhieuNhap ctpn = new ChiTietPhieuNhap(new PhieuNhap(mapn), new SanPham(masp), soLuong);
